...
await capture("question-concern-390x844", { width: 390, height: 844 }, async (page) => {
  ...
  await page.getByRole("heading", { name: /Quando você percebe mais a celulite/ }).waitFor();
});
await capture("insight-personalized-390x844", { width: 390, height: 844 }, async (page) => {
  ...
  await page.getByRole("heading", { name: /Talvez o problema nunca tenha sido falta de vontade/ }).waitFor();
});
await capture("result-cellulite-390x844", { width: 390, height: 844 }, async (page) => {
  ...
  await page.reload({ waitUntil: "networkidle" });
  await page.getByRole("heading", { name: /O seu maior desafio hoje não parece ser a celulite/ }).waitFor();
});
await capture("offer-390x844", { width: 390, height: 844 }, async (page) => {
  ...
  await page.reload({ waitUntil: "networkidle" });
  await page.getByRole("heading", { name: /Nossa sugestão para quem busca/ }).waitFor();
});
